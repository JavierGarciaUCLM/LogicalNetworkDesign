...
class ThemeManager {
  ...
  setTheme(theme) {
    ...
    document.body.style.transition = 'all 0.5s ease'; //Cambiar tema suavemente o rápido
    setTimeout(() => {
...
class CalculadoraSubred {
  ...
  calculaSubnets(baseIp, baseCidr, devices) {
    ...
    for (let i = 0; i < devices.length; i++) {
      ...
      const hostsNecesarios = device.quantity + 2; //+2 para la red y el broadcast, tengo que ponerlo en info!
  ...
  analizarMascaras(subredes) {
    ...
    subredes.forEach(subnet => {
      const key = `/${subnet.cidr}`; //El /$ para "unir" la máscara con la /
      if (!mascaras[key]) {
        mascaras[key] = {
          cidr: subnet.cidr,
    ...
    const mascarasInfo = data.subredes.mascarasUsadas.map(mask => {
      ...
      return `
        <div class="mask-info">
          ...
          <span class="mask-details">${tipos} ${count > 1 ? `(${count} subredes con misma máscara)` : ''}</span>
        </div>
      `;
    }).join('');

    const subnetCards = data.subredes.subredes.map((subnet, index) => {
      const maskUsage = data.subredes.mascarasUsadas.find(m => m.cidr === subnet.cidr); //Se comparte la mascara??
          ...
          <div class="subnet-details">
            ...
            <div class="subnet-row">
              ...
              <div class="mask-container">
                ...
                ${sharedMask ? '<span class="shared-indicator"><i class="fas fa-share-alt"></i> Máscara compartida</span>' : ''}
              </div>
            </div>
    ...
    this.resultSection.innerHTML = `
      <div class="results-content">
        ...
        <div class="summary-stats">
          <div class="summary-card">
            ...
            <div class="summary-info">
              ...
              <small>${data.subredes.subredes.length} subredes creadas</small>
            </div>
          </div>
        ...
        <div class="masks-analysis">
          <h4><i class="fas fa-layer-group"></i> Análisis de Máscaras</h4>
        ...
        <div class="subnets-container">
          <h4><i class="fas fa-list"></i> Configuración de Subredes Consecutivas</h4>
